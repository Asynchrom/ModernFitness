    ...
    <div class="w-full max-w-xs rounded-lg overflow-hidden shadow-xl transform transition-all sm:max-w-lg sm:w-full">
      <form class="bg-white shadow-md rounded-lg px-8 pt-6 pb-8 mb-4">
        ...
        <div v-if="error" class="bg-red-100 border border-red-400 text-red-700 px-4 py-1 mb-4 rounded relative" role="alert">
          <span class="block sm:inline">Experimental feature!</span>
        ...
        <div class="mb-4">
          ...
          <input v-on:keyup.enter="login()" v-model="store.credentials.user.username" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none bg-gray-200 focus:bg-white" id="username" type="text" placeholder="Username">
        </div>
        <div class="mb-6">
          <label class="block text-gray-700 text-sm font-bold mb-2" for="password">
            Password
          </label>
          <input v-on:keyup.enter="login()" v-model="store.credentials.user.password" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 mb-3 leading-tight focus:outline-none bg-gray-200 focus:bg-white" id="password" type="password" placeholder="******************">
        </div>
